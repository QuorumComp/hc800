...
class Font extends Component {
    ...
    def toBitmapChar(in: Array[Int]): Array[Byte] = {
        in.drop(4).take(8).map(v => reverse((v >> 2) & 0xFF).toByte)
    }

    def mapChar(kv : (String, JsValue), mapper: Int => Int): Option[(Int, Array[Byte])] = {
        val (key, value) = kv
        val keyOpt = toInt(key).map(mapper)
        val arrayOpt = value.validate[Array[Int]].asOpt.map(toBitmapChar)
        keyOpt.zip(arrayOpt).headOption
    }

    def readFontJson(): Map[Int,Array[Byte]] = {
        val file = new File("../data/font.txt")
        using (new FileInputStream(file)) { fis =>
            val rootMap = 
                Json.parse(fis)
                .validate[Map[String,JsValue]].asOpt
                .getOrElse(Map.empty[String,JsValue])

            rootMap.flatMap(mapChar(_, windows1252))
        }
    }

    def boxMapper(ch: Int): Int = {
        if (ch >= 'A' && ch <= 'Z') {
            256 + ch - 'A'
        } else {
            256 + ch - 'a' + 26
    ...
    def readBoxJson(): Map[Int,Array[Byte]] = {
        val file = new File("../data/box.txt")
        using (new FileInputStream(file)) { fis =>
            val rootMap = 
                Json.parse(fis)
                .validate[Map[String,JsValue]].asOpt
            ...
            rootMap.flatMap(mapChar(_, boxMapper))
        }
    }

    def readFont(): Array[Byte] = {
        val result = new Array[Byte](512*8)
        val font = readFontJson() ++ readBoxJson()
        val existingChars = font.keys.toSet
        for (key <- 33 to 295) {
            if (!existingChars.contains(key) && !windows1252Missing.contains(key)) {
                println (f"Character $key (0x$key%x) missing")
            }
        }
        for ((key, value) <- font) {
            if (key <= 295) {
                value.copyToArray(result, key * 8, 8)
            }
        }
        result
    }
